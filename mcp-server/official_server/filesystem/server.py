...
def get_session_base_dir(session_id: str) -> Path:
    ...
    if RESOURCE_CONFIG.get("auto_create_dirs", True):
...
def resolve_in_sandbox(user_path: str, session_id: str = None) -> Path:
    ...
    - 支持绝对/相对路径；相对路径相对于session目录或BASE_DIR
    - 解析后必须保证在对应的沙箱目录内
    - 如果提供了session_id，则沙箱根目录为BASE_DIR/session_id
    """
    # 确定沙箱根目录
    if session_id:
        sandbox_root = get_session_base_dir(session_id)
    else:
...
def get_base(session_id: str = None) -> str:
    """获取沙箱根目录。如果提供session_id，返回对应的session目录。"""
    if session_id:
        return str(get_session_base_dir(session_id))
    return str(BASE_DIR)
...
def list_dir(req: ListDirRequest) -> ListDirResult:
    ...
    # 确定sandbox_root用于to_file_info
    sandbox_root = get_session_base_dir(req.session_id) if req.session_id else BASE_DIR
    entries = [to_file_info(p, sandbox_root) for p in paths]
    return ListDirResult(base_dir=str(sandbox_root), entries=entries)
...
def stat(req: StatRequest) -> FileInfo:
    ...
    sandbox_root = get_session_base_dir(req.session_id) if req.session_id else BASE_DIR
    return to_file_info(p, sandbox_root)
